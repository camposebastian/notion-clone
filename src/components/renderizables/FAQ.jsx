...
/* function FAQ({ id, updateEditorContent }) {
  ...
  const handleUpdateQuestion = (frecuentQuestions) => {
    updateEditorContent(id, {
      type: "frecuentQuestions",
      content: frecuentQuestions,
      title: titleQuestion ? newTitle : titleQuestion,
    });
  };
          ...
          <div className="pb-4 flex flex-wrap items-center justify-between max-w-screen-xl m-auto mx-auto gap-y-2 border-b-2 border-[#eaeaea]">
            <div>
              <h2 className="font-bold flex">
                {titleQuestion ? (
                  <input
                    className="font-bold outline-none focus:outline-none w-full w-1/3"
...
export const FaqEditor = createReactBlockSpec(
  ...
  },
  {
    render: (props) => {
      ...
      useEffect(() => {
        const savedEditors = JSON.parse(
          localStorage.getItem("dashboardEditors")
        );
        if (savedEditors.length > 1) {
          let penultimoElemento = savedEditors[savedEditors.length - 2];
          setTitleQuestion(penultimoElemento.content.content);
